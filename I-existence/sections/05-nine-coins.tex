...
\end{example}
\begin{remark}
	An important claim is now in order. Our construction above is not just asleight of hand that leaves everything unchanged. \emph{Every} theory on existence posits that things are ``in some secret way''. Tl\"on's idealist might deny time (our model makes no strong assumption on what time is: discrete, continuous, homogeneous, bent by speed\dots); they live comfortably in $B^\bot$, as defined in \autoref{alcuni_set} without being able to tell anything about how objects ``completely do not exist''.
	...
	In our model $B^\perp, B^\top$ are just particular slices of a single object where they coexist as particular fibers; the approach is \emph{inherently better}, as it yields a quantitative answer, framing both approaches as opposite sides of a spectrum made out of diverse colours.
\end{remark}
...
\begin{example}[La campagna incendiata]\label{incendiata}
	...
	\end{quote}
	(Berkeleyan instantaneism is, of course, way larger than a paragraph: we explore the consequences of this principle in \autoref{di_navi_e_numeri}, in form of a link between Berkeley, the additive nature of truth force in Tl\"on, and its peculiar arithmetics.)
	\begin{center}
		\begin{figure}[h]
