...
\end{remark}
	...
	\[
		...
		\end{cases}
	\]
	In an admissible configuration the subsets $ C_X, C_Y, C_Z $ can only attain an existence $p(g,c,d) \lneq (\top,1)$; that is no coin completely exists \emph{locally}. But for an hypothetical external observer, capable of adding up the forces with which the various parts of $C$ exist, the coins \emph{globally} exist `` in some secret way, of understanding forbidden to men'' \footnote{Non si tratta di un gioco di prestigio che lascia tutto invariato. In ogni discorso sull'esistenza le cose ci sono "in some secret way". L'idealista borgesiano che arriva a negare il tempo (al di là del fatto che non ci siano riferimenti temporali nel modello) sta bello comodo in $B^\bot$ ma non sa dirci nulla sul modo in cui questi oggetti "non esistono" (resp. l'empirista puro in $B^\top$ sul modo in cui "esistono"). Non è la nostra modellizzazione "inusuale", infatti, a mettere in questo imbarazzo l'ontologo. Quello che ora conta è che può stabilire un "grado" di esistenza delle partizioni e determinare qual è il grado di esistenza della loro unione. } (or rather, to $ X, Y, Z $).
	\begin{center}
		\begin{figure}
			...
			\end{tikzpicture}
			\caption{A pictorial representation of the truth forces of coins in different days, a minimal piecewise linear model. $X$ is red, $Z$ is yellow, $Y$ is blue. Time is considered as a continuum marked at weekdays.}
		\end{figure}
	\end{center}
\end{example}
...
\begin{example}[Discontinuity, Lo zaffiro di Taprobana]\label{bli}
	...
	A model for such a universe, where ``terrible consequences'' sometimes follow from the ``impersonal drawings, whose purpose is unclear'' characterising the Company's actions: azioni apparentemente scorrelate tra loro (scagliare ``nelle acque dell'Eufrate uno zaffiro di Taprobana''; sciogliere ``dal tetto d'una torre [\dots\unkern] un uccello''; togliere (o aggiungere) ``un granello di rena ai grani innumerevoli della spiaggia'', quando inserite in una sequenza come in \autoref{}, possono generare per iterazione sistemi dinamici non banali.

	Consideriamo ad esempio il sistema dinamico di insiemi variabili $(\nabla\circ p, [0,1])$ ottenuto dalle iterazioni della composizione $f := \nabla \circ p : [0,1] \to [0,1]$ (si veda \autoref{} per la notazione che chiama $\nabla : I\amalg I \to I$ la fold map di $I$). Siamo in questa situazione 
	\begin{center}
		\begin{figure}
		...
		\end{figure}
	\end{center}
	E' chiaro che il comportamento al limite di una tale successione dipende fortemente dalle proprietà analitiche di continuità di $f$ (cf. \cite{dynsys} per una intro ai sistemi dinamici uno-dimensionali e per le loro applicazioni si veda \cite{,,,}). Una serie ripetuta di sorteggi riesce allora a smuovere in maniera caotica lo spazio delle configurazioni su cui $p$ viene valutata.\footnote{Si possono ovviamente imporre a $p$ miriadi di condizioni di terminazione; la serie dei sorteggi può arrestarsi quando $p$ ha forza massima, o minima, o un prescritto valore, o è fuori di un determinato range di forze\dots{} Le speculazioni e le specializzazioni atte a descrivere un dato fenomeno sono innumerevoli: le lasciamo ai mistagoghi della compagnia, o ai leoni di Qaphqa.}
\end{example}
Un altro esempio compelling è quello della monotonia di una proposizione che dipende, poniamo, da un certo numero di osservatori che acknowledgeano l'``esistenza'' di un oggetto (fisico o concettuale) in gran numero, e da ciò l'esistenza dell'oggetto acquista forza.
\begin{example}[Continuity: a few birds, a horse]\label{blu}
	% Consideriamo l'esempio delle ``tigri di cristallo'' e le ``torri di sangue'' di Tl\"{o}n: oggetti ed entità usuali, diversi da quelli ``classici'' per un dettaglio solo (il colore, la consistenza, il materiale di cui sono composte): la loro esistenza è sfumata, forte meno del massimo, cosicché tigri di carne e una torre di pietra sono tali che $p=(\top,1)$, le loro controparti su Tl\"on esistono con meno forza.
	Consideriamo oggetti la cui forza di esistenza dipende in maniera \emph{monotona} e continua dai loro parametri: per esempio una proposizione $p$ può essere tanto più vera quanta più gente la osserva, perché ``le cose, su Tlön, si duplicano; ma tendono anche a cancellarsi e a  perdere i dettagli quando la gente le dimentichi. È classico l'esempio di  un'antica soglia, che perdurò finchè un mendicante venne a visitarla, e che alla  morte di colui fu perduta di vista. Talvolta pochi uccelli, un cavallo, salvarono le  rovine di un anfiteatro. ''
