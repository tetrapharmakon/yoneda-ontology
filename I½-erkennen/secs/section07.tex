...
\begin{remark}[Ramseyfication and translation functors]\label{carnap_translation_functors}
	...
	We say that a translation adjunction $(L,F)$ is `$\fkK$-admissible' when there is a natural isomorphism $\fkK(L,1)\cong\fkK(1,F)$.
\end{remark}
\color{red}
Qui sarebbe utile cercare di tracciare un parallelo tra l'ammissibilità e la rappresentabilità: mi aspetto che caratterizzare quali funtori $(F,G)$ formano aggiunzioni $\fkK$-ammissibili sia complicato. Ma ci sono proprietà interessanti di questa relazione: per esempio,
\begin{theorem}
	Let $F : \clA \leftrightarrows \clB: G$ be a pair of functors in opposite directions; let $\fkK : \clB \pto \clA$ be a profunctor; if $(F,G)$ is a $\fkK$-admissible pair, then there is a `genuine' adjunction 
