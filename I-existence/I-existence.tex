...
\newenvironment{fo}[1]{\color{blue!30} \textbf{Fouche said}: #1}{}
\newenvironment{de}[1]{\color{blue!30} \textbf{Denta said}: #1}{}
...
\begin{abstract}
  ...
  Such internal language allows to re-enact propositional calculus: all first-order properties of an object can be modeled by propositions $p : U \to\Omega$ valued in an object whose ``elements'' are the truth values that $p$ can assume. Each ontology now $\clE$ yields its own $\Omega_\clE$, and thus its own set of truth values, and thus its own ``theory of meaning''. This approach resembles, but is more general than, fuzzy logics, as most choices of $\clE$ and thus of $\Omega_\clE$ yield nonclassical, many-valued logics.
