...
\begin{remark}\label{alcuni_set}
	...
	\begin{itemize}
		\item the two sets
		      \begin{align*}
			      A^\top  & = \big\{x:U\entails{t(x)} p,\, t(x) > 0\big\}\subseteq U        \\
			      A^\perp & = \big\{x:U \entails{t(x)} \lnot p,\, t(x) > 0\big\}\subseteq U
		      \end{align*}
...
\begin{example}[Nine copper coins]\label{bla}
	...
	\begin{remark}\label{our_beloved_interval}
		In this perspective, \tlon classifier of truth values can be taken as $\Omega_I = \{0<1\}\times I$, where $I$ is any set with more than one element; a minimal example can be $I=\{N,S\}$,\footnote{Justifying this choice from inside \tlon is rather easy: the planet is an hyperboloid with two connected components, each of which has its own logic independent from the other.} but as explained below \autoref{something_on_I} a more natural choice for our purposes is the closed real interval $I=[0,1]$.
...
\begin{example}[Continuity: a few birds, a horse]\label{blu}
	...
	\[(R,n)\entails{1-1/n} p.\]%\textstyle p(R, n) = \big(\top, 1-\frac{1}{n}\big)\]
	\begin{figure}[h]
		\begin{center}
