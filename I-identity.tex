...
% \[
%   \begin{tikzpicture}[xscale=2,>=stealth, baseline=(current bounding box.center)]
%   \node (1) at (0,0) {$[1]$};
%   \node (0) at (-1,0) {$[0]$};
%   \node (2) at (1,0) {$[2]$};
%   \draw[->] (0) --node[below] {$e$} (1);
%   \draw[->] (2) --node[below] {$m$} (1);

%   \path (1) edge[loop above, min distance=1cm, ->] node[above] {$i$} (1);
% \end{tikzpicture} 
% \]
