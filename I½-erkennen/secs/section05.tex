...
\color{red}
The following remark shows how new structure comes `almost for free' when things are interpreted this way. Assume $\phi : \clT \to \clW$ and $\psi : \clO \to \clW$ are canvases, $\fkR$ is a Ramsey map, and $\Lan_{y_{\clO}}\hat R$ the functor corresponding to $\fkR$ under the construction in \eqref{lalan}; in this notation, we can state a condition of compatibility between the theory identified by $(\phi,\psi)$ and te Ramsey map $\fkR$.
\begin{remark}[Inducing an hermeneutics]\label{inducing_herme}
Consider the diagram
\[ \vcenter{\xymatrix{
	& \clW \ar[dl]_{N_\phi} \ar[dr]^{N_\psi} & \\ 
	[\clT^\op,\Set] \ar[rr]_R && [\clO^\op,\Set]
}}	 \label{discuss_0}\]
given by the theoretical and observational nerves. We seek sufficient conditions in order for \eqref{discuss_0} to be filled by a meaningful 2-cell $N_\psi \To R\diamond N_\phi$. Observe that the two functors
\[
\vcenter{\xymatrix{
	**[l] \clO^\op \times \clT \rtwocell^{N_\psi\diamond\phi_*}_{R}{\alpha}& \Set
}}	
\label{discuss_2}\]
where the functor $N_\psi\diamond\phi_*$ is just $\clW(\psi,\phi)$, are parallel: from basic properties of the mating operation we can deduce the existence of a chain of isomorphisms 
\[
\begin{array}{c}
	N_\psi\diamond\phi_* \To R \\ \hline 
	N_\psi\To \Ran_{\phi_*}R \\ \hline 
	N_\psi \To R\diamond N_\phi
\end{array}	
\label{discuss_3}\]
so that a 2-cell $N_\psi\diamond\phi_* \To R$ can uniquely be obtained from a 2-cell $N_\psi \To R(\phi,1)$, i.e. by a natural transformation
...
\begin{definition}
	...
	\begin{itemize}
		\item A \emph{fundamental cell} is a natural transformation $N_\psi \To R(\phi,1)$;
		\item In $\clW$ \emph{hermeneutics is possible} if the right Kan extension $\bk{\phi,\psi} := \Ran_{ N_\phi}N_\psi$ exists \emph{as a functor} (note that it always exists as a profunctor).
	\end{itemize}
