...
\begin{remark}\label{alcuni_set}
	Given that $I=[0,1]$ endowed with its usual Euclidean topology is one of our most natural choices, we explicitly define some interesting sets obtained out of $\Omega_I$ or out of a proposition $p : U \to \Omega_I$ in the Mitchell-Bénabou language of $\Set/[0,1]$:%definiamo alcuni insiemi di interesse per una data proposizione $p : U \to \Omega_I$ per questa scelta di $I$:
	\begin{itemize}
		...
		\item $B^\top = \{x : U \mid p(x) = (1,1)\} = p^\leftarrow((1,1))$ and $B^\perp = \{x : U \mid p(x) = (0,1)\}$ ``true''s and ``false''s; these are obtained as fibers over the maximal element of $I$.
