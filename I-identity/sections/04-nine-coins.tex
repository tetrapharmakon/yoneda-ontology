...
\begin{example}[Nine copper coins]
  First, we recall the exact statement of the paradox from \cite{}:\footnote{The translation we employ is classical and comes from \cite{}: 
  \begin{quote}
...
\end{quote}
  ...
  \begin{itemize}
    ...
    c_{Y1},c_{Y2},c_{Y3},c_{Y4}\}$, $C_Z= \{c_{Z1}, c_{Z2}, c_{Z3}\}$ As already said, the truth multiplier $I$ is the closed interval $[0,1]$ with its canonical order --so with its canonical structure of Heyting algebra, and if needed, endowed with the usual topology inherited by the real line.%;  we do notnon operiamo l'identificazione $(\top,0) \sim (\perp,1)$ o $(\perp,0) = (\top,1)$.
    \item Propositions of interest for us are of the following form:% Le proposizioni di cui ci occupiamo sono della forma
...
\end{center}
\end{example}
\begin{example}[Lo zaffiro di Taprobana]
...
\end{example}
\begin{example}[Pochi uccelli, un cavallo]
