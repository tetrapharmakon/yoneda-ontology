...
From $\clT_L$ it is possible to carve a cartesian closed category, whose obects are denoted $\tilde{b}$ as $b$ runs over the basic types of $L$. Then, we define recursively a \emph{semantic translation} operation from basic types $\trn{B} := B$ and
...
\begin{enumtag}{st}
	...
	\item the category of functors $\Qat[T_\clC, \clS]$, whose codomain is a Yuggoth.
\end{enumtag}
