...
Più semplicemente, usando def 2.1, e considerando che $\clO \subset \clT$, ci pare che la ramseyfication, non potendo nessun termine $\tau_k \in T$ uscire dal vocabolario teorico, sia un ri-tradurre nel linguaggio di $\clO$ il contenuto della teoria, sostituendo i termini teorici "puri" con delle variabili \cite{?}.
\begin{definition}
	Given an \emph{application function} $\psi: \clT \to \clO$, a Ramsey-sentence of a theory $T$ is 
	\[ \overline{T} = \{[t_i/x_i], \psi (\varphi (\omega_j))\} 
	\]
\end{definition}
